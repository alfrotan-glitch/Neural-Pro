...
ok(project.includes('const nextTracks = createDedicatedTimelineTrack(tracks, laneRole, newClip);'), 'asset insertion must create a dedicated track');
ok(subscribe.includes("createDedicatedTimelineTrack(tracks, 'subscribe', newClip)"), 'subscribe must use a dedicated lane');
ok(inspector.includes("createDedicatedTimelineTrack(tracks, 'caption', newClip)"), 'manual caption must use a dedicated lane');

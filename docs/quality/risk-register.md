...
| **R-048** | **F-3 — source duration answers two different questions.** With persisted metadata it is `persisted − trim.in` (media remaining); without it, `trim.out − trim.in` (the trim window) | P1 | Medium | clip length changes depending on which metadata happens to be persisted | WP-11 | decide the intended semantics; both branches are pinned by characterization tests in `tests/domain-core/duration.test.ts` | OPEN |

...
| **R-046** | **F-1 / F-1b — `NaN` propagation in the time authorities.** `mediaTimeMapper.projectTimeToSourceTime` returns `NaN` for a non-finite project time (`Math.max(0, NaN)`), and callers assign it directly to `videoEl.currentTime` (`mediaSyncController.ts:34`, `playbackService.ts:44`); `projectDuration.clampProjectTime(t, NaN)` returns `NaN` and feeds `currentTime` | P2 | Medium | a single non-finite playhead silently breaks preview seek and can corrupt the rendered timeline | WP-11 | adopt `duration.projectTimeToSourceTime` / `duration.clampProjectTime`; divergence pinned in `tests/domain-core/parity.test.ts` | OPEN |
| **R-047** | **F-2 — transform composition order.** The preview emitter `getPreviewTransformCss` emits `T · S · R`; the canonical order is `T · R · S`. Identical for a uniform scale; divergent when `scaleX ≠ scaleY` **and** `rotation ≠ 0` — the D-004 case (measured drift 662.019 px) | P1 | High | preview and export disagree for rotated, non-uniformly scaled clips | WP-03 | adopt `transform.transformToCss`; verify with `audit/repro-transform-order.mts` before switching | OPEN |
| **R-048** | **F-3 — DECIDED 2026-09-09 (owner): canonical clip source duration is the TRIM WINDOW.** Persisted media metadata is redefined as the asset's intrinsic duration (validation only, never a clip bound). The canonical core now expresses this; **downstream production modules are unchanged until WP-11 integrates** — until then the legacy `persisted − trim.in` behaviour still executes | P1 | Medium | during the gap, the contract and the executing code disagree: a clip with `trim {2, 9}`, `duration 20`, `sourceMediaDuration 12` is **10 s** in production and **7 s** by contract. No user-visible change occurs until WP-11 switches call sites | WP-11 (integration) | adopt `duration.getTrimDuration` / `getEffectiveClipDuration` / `getTimelineDuration`; verify against real projects, especially unbounded-media clips (image/text) that carry a trim window, which the canonical rule now bounds | **OPEN — decision made, integration pending** |
| **R-049** | **F-4 — two visible-time-range implementations disagree.** `timelineGeometry.getVisibleTimeRange` uses the raw viewport width; `timelineViewportGeometry.getVisibleTimelineTimeRange` subtracts `TIMELINE_HEADER_WIDTH` (160 px). One is wrong by 160 px of timeline time | P2 | Medium | clips culled too early or rendered too late while scrolling | WP-08 / WP-11 | pick one (the header-aware one) and delegate | OPEN |
| **R-050** | **Canonical core is not yet wired in.** No production module imports `src/domain/core/**`; the legacy authorities still execute, so the consolidation is a plan rather than a fact | P2 | Medium | the kernel drifts from the code it is supposed to canonicalise | Core Architecture (adoption: WP-03/05/08/11) | parity suite fails on drift; SHIM-007 expires at WP-12 | OPEN |

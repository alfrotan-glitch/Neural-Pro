...
| **INV-021** | `src/domain/core/**` is pure: no React, DOM, storage, timers, `fetch`, `crypto`, `process.env`, `Math.random`, `Date.now`, and no import outside `src/domain/**` | `npx tsx tests/domain-core/run.ts` → `purity` suite | static (labelled) | Core Architecture | **PASS** | purity suite green; `src/domain/core/**` exists as of ADR-017 |
| **INV-022** | The canonical kernel and the authorities that execute today produce identical values for clip duration, source range, playback rate, time mapping, project duration, clamping, transform normalisation, origin, translation and the media frame | `npx tsx tests/domain-core/run.ts` → `parity` suite (deterministic clip matrix, divergences F-1/F-1b pinned explicitly) | executable | Core Architecture | **PASS** | parity suite green |
| **INV-023** | There is exactly one framerate resolution rule: `resolveRenderFps(export, project)` with explicit precedence, and captions derive from it | `npx tsx tests/domain-core/run.ts` → `fps` suite | executable | Core Architecture | **PASS** (rule defined and tested) | **INV-013 remains FAIL** — the three product fps sources do not consume the rule yet; that is WP-11's adoption |
| **INV-024** | Clip intervals are half-open `[start, end)`; adjacent clips never both render at the shared boundary | `npx tsx tests/domain-core/run.ts` → `time` and `duration` suites | executable | Core Architecture | **PASS** | |
| **INV-025** | Derived values are never stored: `totalDuration` and `effectiveDuration` are recomputed by every mutator; a normaliser never emits a non-finite value | `npx tsx tests/domain-core/run.ts` → `project`, `duration` and `geometry` suites | executable | Core Architecture | **PASS** | |
| **INV-026** | Duration vocabulary is separated: media intrinsic duration (asset) · trim duration (`trim.out − trim.in`, the canonical source duration) · effective clip duration (÷ rate) · timeline duration (`min(declared, effective)`). No function conflates two of them, and media intrinsic duration never bounds a clip | `npx tsx tests/domain-core/run.ts` → `duration` suite; F-3 divergence pinned in `parity` suite | executable | Core Architecture | **PASS** (canonical core) / **FAIL** in production until WP-11 integrates | core suites green; `clipTimelineDuration.getCanonicalClipSourceDuration` still returns `persisted − trim.in` |
| **INV-027** | `src/domain/**` imports nothing outside `src/domain/**` (no edge to `features/**`, `components/**`, `core/**`, `store/**`, `lib/**`, `config/**` or the deleted root type barrels) | `npx tsx tests/domain-core/run.ts` → `boundaries` suite; negative controls proven by injected violations | static (labelled) | Core Architecture | **PASS** | boundaries suite green |
| **INV-028** | Every compatibility shim owned by the core carries an ID, owner, removal milestone and marker at its definition site, and fails the suite when its milestone ships | `npx tsx tests/domain-core/run.ts` → `shims` suite | static (labelled) | Core Architecture | **PASS** | SHIM-006, SHIM-007 registered in ADR-013 and marked in code |
| **INV-029** | Every exported **value** of `src/domain/core/**` is directly exercised by a test; the kernel has no untested public rule | `npx tsx tests/domain-core/run.ts` → `coverage` suite (negative control: an added orphan export fails it) | static (labelled) | Core Architecture | **PASS** | 93 exported values, 0 untested |
| **INV-030** | The kernel throws exactly one error type: every validator throws `DomainInvariantError` with a stable `code` and a named `field` | `npx tsx tests/domain-core/run.ts` → `validation` and `identity` suites | executable | Core Architecture | **PASS** | 9 distinct violation codes asserted; a plain `Error` fails the suite |
